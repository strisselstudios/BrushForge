...
public sealed class TreeGeneratorTests
{
    ...
    public void GenerateCreatesTrunkAndRequestedCanopyLayers()
    {
        ...
        Assert.Equal(260, branchParts.Length);
        Assert.Equal(4, canopyParts.Length);
        Assert.Equal(267, result.BrushCount);
        Assert.All(
    ...
    [Fact]
    public void GenerateRealizesTwentySegmentedValidTertiaryBranchPathsAtFullDetail()
    {
        TreeGenerationResult result =
            TreeGenerator.Generate(
                CreateSettings(
        ...
        GeneratedTreeBrush[] tertiaries =
            GetTertiaryBranchParts(result);
        IGrouping<string, GeneratedTreeBrush>[] paths =
            tertiaries
                .GroupBy(
                    branch =>
        ...
        Assert.Equal(80, tertiaries.Length);
    ...
    [Fact]
    public void GenerateMakesTertiaryBranchesGrowOutwardFromTrunk()
    {
        for (ulong generationSeed = 1UL; generationSeed <= 16UL; generationSeed++) {
            TreeGenerationResult result =
                TreeGenerator.Generate(
                    CreateSettings(
                        generationSeed: generationSeed,
    ...
    [Fact]
    public void GeneratePreventsNetDownwardTertiaryGrowth()
    {
        for (ulong generationSeed = 1UL; generationSeed <= 16UL; generationSeed++) {
            TreeGenerationResult result =
                TreeGenerator.Generate(
    ...
    [Fact]
    public void GenerateKeepsTerminalBranchletsHiddenBelowHighDetailRange()
    {
        TreeGenerationResult result =
            TreeGenerator.Generate(
                CreateSettings(
                    generationSeed: 71_906UL,
                    detail: 0.93));

        Assert.Empty(
            GetTerminalBranchletParts(result));
    }

    [Fact]
    public void GenerateRealizesTwoSegmentTerminalBranchletsAtFullDetail()
    {
        TreeGenerationResult result =
            TreeGenerator.Generate(
                CreateSettings(
                    generationSeed: 91_704UL,
                    detail: 1.0));
        GeneratedTreeBrush[] branchlets =
            GetTerminalBranchletParts(result);
        IGrouping<string, GeneratedTreeBrush>[] paths =
            branchlets
                .GroupBy(
        ...
        Assert.Equal(80, branchlets.Length);
    ...
    private static GeneratedTreeBrush[] GetTertiaryBranchParts(
        TreeGenerationResult result)
    {
        return result.Parts
            .Where(
    ...
    private static GeneratedTreeBrush[] GetTerminalBranchletParts(
        TreeGenerationResult result)
    {
        return result.Parts
            .Where(
